...
let joinStreams = async () => {
    ...
    client.on("volume-indicator", function(evt){
        for (let i = 0; evt.length > i; i++){
            ...
            if(volume > 0){
                document.getElementById(`volume-${speaker}`).src = './assets/volume-on.jpg'
            }else{
                document.getElementById(`volume-${speaker}`).src = './assets/volume-off.jpg'
            }
    ...
    let player = `<div class="video-containers" id="video-wrapper-${config.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${config.uid}" src="./assets/volume-on.jpg" /> ${config.uid}</p>
...
let handleUserJoined = async (user, mediaType) => {
    ...
    if (mediaType === 'video'){
        ...
        player = `<div class="video-containers" id="video-wrapper-${user.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${user.uid}" src="./assets/volume-on.jpg" /> ${user.uid}</p>
